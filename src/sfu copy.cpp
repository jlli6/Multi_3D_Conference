...
public:
    ...
    // 确认所有客户端连接，并广播 "READY"
    void sync_client(uint64_t stream_id = 9,int ready_times = 1) {
    ...
    void modelDownloadPhase(uint64_t stream_id = 7) {
        ...
        for (auto& kv : clientConnections) {
            ...
            // 打开流
            if (conn->open_stream(stream_id) < 0) {
                std::cerr << "SFU: 为客户端 " << clientId << " 打开模型传输流失败" << std::endl;
                continue;
            }
            ...
            // 关闭流
            quiche_conn_stream_shutdown(conn->conn, stream_id, QUICHE_SHUTDOWN_WRITE, 0); // Close write operation
            quiche_conn_stream_shutdown(conn->conn, stream_id, QUICHE_SHUTDOWN_READ, 0);  // Close read operation
            std::cout << "SFU: 已完成向客户端 " << clientId << " 的模型数据发送" << std::endl;
        }
    
        std::cout << "SFU: 模型下载阶段完成" << std::endl;
    }
...
int main(int argc, char* argv[]) {
    ...
    // sleep(1);
    // sfu.confirmAllConnection2();
    // sleep(5);

    // sfu.modelUploadPhase(7);
    
    // sleep(1);  // 等待 1 秒，确保所有客户端连接完成
    std::chrono::milliseconds ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch());
    std::cout << "current time: " << ms.count() << std::endl;
    // std::cout << "SFU: 再上传一次" << std::endl;
    // sfu.modelUploadPhase(9);
